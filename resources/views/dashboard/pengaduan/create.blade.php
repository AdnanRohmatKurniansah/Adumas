...
<div class="container grid px-6 mx-auto">
    <h4 class="my-6 text-lg font-semibold text-gray-600 dark:text-gray-300">Tambah pengaduan</h4>
    <form action="/dashboard/pengaduan" method="post" enctype="multipart/form-data">
        ...
        <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800" style="max-width: 900px">
            <label class="block mt-2 text-sm">
                ...
                <input id="judul" required value="{{ old('judul') }}" class="block w-full mt-1 text-sm dark:text-gray-300 dark:bg-gray-700 focus:outline-none form-input @error('judul') is-invalid @enderror" type="text" name="judul" placeholder="Judul"/>
                @error('judul')
            ...
            <label class="block mt-2 text-sm">
                ...
                <input id="slug" required value="{{ old('slug') }}" class="block w-full mt-1 text-sm dark:text-gray-300 dark:bg-gray-700 focus:outline-none form-input @error('slug') is-invalid @enderror" type="text" name="slug" placeholder="Slug"/>
                @error('slug')
                    <span class="text-xs text-red-600 dark:text-red-400">
                        {{ $message }}
                    </span>
            ...
            <label class="block mt-4 text-sm">
                ...
                <input required  class="block w-full mt-1 text-sm dark:text-gray-300 dark:bg-gray-700 focus:outline-none form-input @error('foto') is-invalid @enderror" type="file" id="foto" name="foto" onchange="previewFoto()"/>
                @error('foto')
                    <span class="text-xs text-red-600 dark:text-red-400">
                        {{ $message }}
                    </span>
